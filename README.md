...
**8d. Captioning** (`captioner.py`) — prefers Suno's `timed_lyrics.json` for exact word timing; falls back to local Whisper if missing. Renders **ASS karaoke subtitles**: white base sentence with the currently sung word highlighted in red and slightly larger. Per-word display capped at 1.5s. Optional `--no-karaoke` mode shows only the sung word, solo and centered
...
|---|---|
| `final_captioned.mp4` | Skip story entirely |
